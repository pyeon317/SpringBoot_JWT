...
public class JWTService {
	...
	public static String createToken(String username) {
		...
		//만료시간 
		long expire = System.currentTimeMillis() + 360000; //1시간 뒤
		...
		Builder builder = JWT.create().withSubject(username) //주제
					...
					.withClaim("admin", "공개 클레임 ㅇㅇㅇ~"); //+공개클레임
					
		return builder.sign(alg);//빌더객체생성
	}
	...
	public static boolean validateToken(String token) throws JWTVerificationException {
		...
		verifier.verify(token); //토큰 검사 - 만료기간 or 토큰위조가 발생하면 throws 처리됩니다.
